      ...
      bind: (..._args: any[]) => ({
        ...
        run: async () => {
          ...
          if (sql.includes('UPDATE sessions SET status')) {
            ...
            if (row) {
              row.status = _args[0] === token ? (_args[0] === 'error' ? 'error' : 'ready') : row.status;
          ...
          if (sql.includes('UPDATE identity_profiles')) {
            ...
            const row = currentRows.find((r) => r.session_token === sessionToken);
...
export function mockR2(objects: { key: string; size: number; body?: ArrayBuffer }[] = []): R2Bucket {
  return {
...
export function createGemmaResponse(description?: string) {
  return {
    response: description ?? 'Mock gemma-4 description: male in early 20s, oval face, dark brown almond eyes, low nose bridge, defined jawline, medium skin tone, short silver-grey hair.',
  };
}
