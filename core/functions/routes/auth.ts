...
authApp.post("/magic-link", async (c) => {
  ...
  if (!email || !EMAIL_REGEX.test(email)) {
    return c.json(
      badRequest("INVALID_EMAIL", "A valid email address is required."),
      422,
    );
  }
  ...
  try {
    ...
    // Send email. The token is stored first so retries can use the same request path.
    ...
    return c.json(
      serviceUnavailable("MAGIC_LINK_FAILED", "Failed to send sign-in link.", {
        error: message,
      }),
      503,
    );
...
authApp.get("/verify", async (c) => {
  ...
  try {
    ...
    await c.env.DB.prepare("UPDATE magic_links SET used = 1 WHERE token = ?1")
      .bind(token)
      .run();
    ...
    await c.env.DB.prepare("DELETE FROM magic_links WHERE token = ?1")
      .bind(token)
      .run();
    ...
    const message = err instanceof Error ? err.message : "Unknown error";
    return c.json(
      serviceUnavailable("VERIFY_FAILED", "Could not verify sign-in link.", {
        error: message,
      }),
      503,
    );
...
authApp.get("/me", requireAuth, async (c) => {
  ...
  try {
    ...
    const message = err instanceof Error ? err.message : "Unknown error";
    return c.json(
      serviceUnavailable("ME_LOOKUP_FAILED", "Could not retrieve user info.", {
        error: message,
      }),
      503,
    );
...
authApp.post("/logout", async (c) => {
  ...
  if (sessionToken) {
    try {
      await c.env.DB.prepare("DELETE FROM sessions_auth WHERE id = ?1")
        .bind(sessionToken)
        .run();
    } catch {
