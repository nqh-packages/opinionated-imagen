...
describe('POST /api/profile/build', () => {
  ...
  beforeEach(() => {
    env = {
      ...
      STORAGE: mockR2(),
      AI: mockAi({
        '@cf/google/gemma-4-26b-a4b-it': createGemmaResponse(),
  ...
  it('builds profile and transitions to building_profile', async () => {
    ...
    env.STORAGE = mockR2(createSelfieObjects(10));

    const res = await app.request('/api/profile/build', {
      method: 'POST',
  ...
  it('rejects build when fewer than 10 selfies uploaded', async () => {
    const session = createSession({ selfie_count: 6 });
    env.DB = mockD1([session]);
    ...
    expect(body.context?.current).toBe(6);
    expect(body.context?.needed).toBe(10);
  });

  // 3. Prevention: not enough moodboard photos
  it('rejects build when fewer than 5 moodboard photos uploaded', async () => {
    ...
    expect(res.status).toBe(422);
    const body = await res.json() as any;
    expect(body.error_code).toBe('NOT_ENOUGH_MOODBOARD');
  });
