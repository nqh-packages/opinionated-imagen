...
function main() {
  const { command, root, productId } = parseArgs(process.argv.slice(2));
  try {
    if (command === 'validate') {
    ...
    throw productError('UNKNOWN_COMMAND', command, `unknown command "${command}"`, 'use validate or compile');
  } catch (error) {
...
export { compile, loadProducts, validate };
